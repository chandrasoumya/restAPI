...
Router.put("/posts/:id/like", async (req, res) => {
    try {
        ...
        else if(!post.Likes.includes(req.body.userName)){ // here the user name is of who is going to like
...
Router.put("/posts/:id/comment", async (req, res) => {
    try {
        ...
        if (!post) {
            ...
            post.Comments.push({commentId: uuid.v4(),
...
Router.put("/posts/:id/comment/:commentId", async (req, res) => {
    try {
        ...
        if (!post) {
            ...
            post.Comments = post.Comments.filter((item) => item.commentId !== req.params.commentId);
            await Post.updateOne(post);
